...
export default function SideNav() {
  ...
  const handleClick = async (item: (typeof NAV_ITEMS)[number]) => {
    if (item.type === 'scroll') {
  ...
  return (
    <nav className="hidden md:block sticky top-8 w-44 shrink-0 self-start">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <li key={item.id}>
              ...
              >
                {icons[item.id]}
                {item.id === 'share' && copied ? '已複製!' : item.label}
              </button>
            </li>
          );
        })}
